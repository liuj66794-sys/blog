...
<div class="prep-exam-strip">
  <span class="prep-exam-strip__item" title="学习计划推算目标，正式考期以官方通知为准">计划目标日 <strong>${examDate}</strong></span>
  <span class="prep-exam-strip__item prep-exam-strip__countdown" id="exam-countdown" data-exam="${examDate}"></span>
  <span class="prep-exam-strip__item" id="prep-now" ${nowAttrs}></span>
</div>
...
const main = () => {
  ...
  console.log(execFileSync(process.execPath, [path.join(ROOT,'scripts/generate-question-index.mjs')], {cwd:ROOT,encoding:'utf8'}).trim())
  console.log(execFileSync(process.execPath, [path.join(ROOT,'scripts/generate-lesson-search.mjs')], {cwd:ROOT,encoding:'utf8'}).trim())
