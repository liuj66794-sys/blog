...
(function () {
  ...
  function setupQuiz(quiz, index) {
    ...
    function restore(state) {
      ...
      opts.forEach(function (button) {
        ...
        button.classList.toggle('wrong', state.attempts.includes(button.dataset.k) && button.dataset.k !== answer);
      });
      if (verdict && state.attempts.length) {
        verdict.textContent = state.done
          ? (state.attempts.length === 1 ? '✓ 一次答对，漂亮！' : '✓ 答对了（第一次选错了，别急，看清错在哪）')
          : '✗ 再想想——先别急着看解析。';
  ...
  ZC.refreshProgress = function () {
    document.querySelectorAll('.lesson-progress').forEach(function (bar) {
      ...
      if (tq > 0) parts.push('本轮随堂测 ' + quizzes.filter(function (q) { return q && q.done; }).length + '/' + tq + ' 已完成（首次答对 ' + quizzes.filter(function (q) { return q && q.done && q.attempts.length === 1; }).length + '）');
