...
// Formatting enrichment must not reset existing mastery or an in-progress group.
export function questionSignature(q) { return JSON.stringify([q.stem, q.options?.map(({ value, text }) => ({ value, text })), q.answer]) }
